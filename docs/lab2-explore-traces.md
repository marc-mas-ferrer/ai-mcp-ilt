...
assets/images/service_health.png
...
assets/images/explore_prompts.png

### 3.2 Open distributed traces

Select **View traces** in the upper-right corner.

assets/images/view_traces.png

The Distributed Tracing app opens and displays spans associated with the selected service.

assets/images/spans_list.png

### 3.3 Select a trace

Open a trace associated with the `/chat` endpoint.

assets/images/trace_dive.png
...
The exact names of automatically generated spans may vary depending on the versions of LangChain, Traceloop, OpenTelemetry, and LiteLLM used in the workshop.
...
In this workshop, the application requests the LiteLLM alias `workshop-chat`, which routes to Amazon Nova Micro through Amazon Bedrock.
...
Compare the short intent-classification call with the final response-generation call.
...
## 💻 Step 5: Understand Local Embeddings

The application uses the local `sentence-transformers/all-MiniLM-L6-v2` model to generate embeddings inside the Codespace.

It does not send embedding requests to LiteLLM or Amazon Bedrock.

### 5.1 Examine document retrieval

Locate the `retrieve_documents.task` span.
...
Because embedding generation is local, you should not expect a remote `openai.embeddings` span or hosted-model token attributes for this operation.

### 5.2 Compare local and hosted embeddings

| Local embeddings in this workshop | Hosted embedding service |
|---|---|
| Runs inside the Codespace | Runs through a remote API |
| No API key required | Requires authentication |
| No per-token model cost | May have a token or request cost |
| No remote LLM span | Usually produces a remote model span |
| Avoids network latency | Includes network and provider latency |

The local embedding model still performs an essential part of the RAG pipeline, even though it does not produce the same telemetry as a hosted embedding service.
...
The instructor has uploaded a lookup table containing the model limits used by this workshop.
...
The result should contain the model identifiers and their maximum input and output token values.

The lookup contains both the LiteLLM alias and the underlying Bedrock model identifier so that the query works with either value recorded in `gen_ai.response.model`.
...
Try changing the visualisation to **Pie**.

This query also helps confirm which identifier the instrumentation records for the model.
...
Try changing the visualisation to **Categorical**.

Look for the operations with the highest average duration.
...
### 9.1 Understand the model price

The workshop uses Amazon Nova Micro through Amazon Bedrock.

| Model | Input cost per 1 million tokens | Output cost per 1 million tokens |
|---|---:|---:|
| Amazon Nova Micro | $0.035 | $0.14 |

Local embeddings have no hosted-model token cost.

### 9.2 Inspect the pricing lookup table
...
The lookup table maps the possible model identifiers to their input and output prices.

Using a lookup table means the analysis query does not need to hardcode the prices. The instructor can replace the lookup data if the model or pricing changes.

### 9.3 Calculate estimated token cost
...
The estimated value will be very small because Nova Micro is inexpensive and the workshop generates limited traffic.

The important lesson is the calculation pattern:
...
### 9.4 Identify the largest prompts
...
### 9.5 Analyse token usage over time
...
### 9.6 What to do with token data
...
- [ ] Find `ai-chat-service-{YOUR_ATTENDEE_ID}`
- [ ] Open a trace for the `/chat` endpoint
- [ ] Identify the RAG workflow and task spans
- [ ] Find both LLM calls in the RAG workflow
- [ ] Inspect available prompts, completions, and token attributes
- [ ] Explain why local embedding generation does not create a remote LLM span
- [ ] Find the ChromaDB vector-search span
- [ ] Load `/lookups/ai/bedrock/model-max-tokens`
- [ ] Calculate average token utilisation
- [ ] Load `/lookups/ai/bedrock/model-costs`
...
Confirm that the value in `gen_ai.response.model` exists in the lookup table's `model` field.

Also confirm that your account has permission to read Grail lookup files.
...
1. Follow a RAG request through its complete trace
2. Distinguish workflow, task, vector-store, and LLM spans
3. Inspect prompts, completions, token usage, and latency
4. Understand the role of local embedding generation
5. Analyse document retrieval through the ChromaDB span
6. Identify large prompts and expensive stages

**When a RAG response is poor:** inspect the retrieved documents, generated context, final prompt, and completion before changing the model.
...
1. Analyse model usage with DQL
2. Compare token usage with model limits
3. Calculate estimated cost using a Grail lookup table
4. Identify the requests with the largest prompts
5. Track input tokens, output tokens, and request volume over time
6. Reuse the analysis in dashboards, alerts, and Workflows
